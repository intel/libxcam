...
void debug_print_3a_stats (XCam3AStats *stats_ptr)
{
    static int line = 0;
    line++;
    printf ("********%dline debug 3a stats(%dbits) \n", line, stats_ptr->info.bit_depth);
    for (int y = 30; y < 60; ++y) {
        printf ("---- y ");
        for (int x = 40; x < 80; ++x)
            printf ("%4d ", stats_ptr->stats[y * stats_ptr->info.aligned_width + x].avg_y / 16);
        printf ("\n");
    }
    ...
    for (int y = 30; y < 60; ++y) {
        printf ("---- GR ");
        for (int x = 40; x < 80; ++x)
            printf ("%4d ", stats_ptr->stats[y * stats_ptr->info.aligned_width + x].avg_gr / 16);
        printf ("\n");
    }
