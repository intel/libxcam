...
CLImageScaler::prepare_scaler_buf (const VideoBufferInfo &video_info, SmartPtr<DrmBoBuffer> &output)
{
    ...
    if (!_scaler_buf_pool.ptr ()) {
        ...
        uint32_t new_width = XCAM_ALIGN_UP ((uint32_t)(video_info.width * _scaler_factor), 2 * XCAM_CL_IMAGE_SCALER_KERNEL_LOCAL_WORK_SIZE);
        uint32_t new_height = XCAM_ALIGN_UP ((uint32_t)(video_info.height * _scaler_factor), 2 * XCAM_CL_IMAGE_SCALER_KERNEL_LOCAL_WORK_SIZE);
...
#include "kernel_image_scaler.clx"
    ...
    if (V4L2_PIX_FMT_NV12 == format) {
        SmartPtr<CLImageKernel> uv_scaler_kernel = new CLImageScalerKernel (context, CL_IMAGE_SCALER_NV12_UV, scaler_handler);
        {
            XCAM_CL_KERNEL_FUNC_SOURCE_BEGIN(kernel_image_scaler)
#include "kernel_image_scaler.clx"
            XCAM_CL_KERNEL_FUNC_END;
