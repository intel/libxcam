...
CLHdrImageKernel::CLHdrImageKernel (SmartPtr<CLContext> &context, const char *name)
...
#include "kernel_hdr_lab.cl"
    ...
    if (type == CL_HDR_TYPE_RGB) {
        hdr_kernel = new CLHdrImageKernel (context, "kernel_hdr_rgb");
        ret = hdr_kernel->load_from_source (kernel_hdr_rgb_body, strlen (kernel_hdr_rgb_body));
    }
    else if (type == CL_HDR_TYPE_LAB) {
        hdr_kernel = new CLHdrImageKernel (context, "kernel_hdr_lab");
