...
public:
    ...
    XCamReturn parse_calib_file (const char *file_path, std::vector<CalibrationInfo> &calib_info, int32_t camera_count);
    XCamReturn parse_calib_param (char *file_body, std::vector<CalibrationInfo> &calib_info, int32_t camera_count);

#if HAVE_JSON
    //file generated by OpenCV fisheye camera calibration
    XCamReturn parse_fisheye_camera_param (const char *file_path, FisheyeInfo *fisheye_info, int32_t camera_count);
#endif
