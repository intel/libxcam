 ...
 * thr_C:            Motion sensitivity for UV, higher value can cause more motion blur
 */

 __kernel void kernel_tnr_yuv(__read_only image2d_t inputFrame, __read_only image2d_t inputFrame0, __write_only image2d_t outputFrame, uint vertical_offset, float gain, float thr_Y, float thr_C) 
{                     
    ...
    float coeff_U = 1.0;    
    if (diff_U < thr_C) coeff_U =  gain;                                                 
                
    float coeff_V = 1.0;      
    if (diff_V < thr_C) coeff_V =  gain;                                                 
