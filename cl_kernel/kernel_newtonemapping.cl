...
__kernel void kernel_newtonemapping (__read_only image2d_t input, __write_only image2d_t output, __global float *hist_leq, int image_width, int image_height)
{
    ...
    for(int i = 0; i < BLOCK_FACTOR; i++)
    {
        for(int j = 0; j < BLOCK_FACTOR; j++)
        {
            ...
            haleq.x = hist_leq[start_index + (int)(src_y_data.x * 65535 + 0.5f) / 16];
            haleq.y = hist_leq[start_index + (int)(src_y_data.y * 65535 + 0.5f) / 16];
            haleq.z = hist_leq[start_index + (int)(src_y_data.z * 65535 + 0.5f) / 16];
            haleq.w = hist_leq[start_index + (int)(src_y_data.w * 65535 + 0.5f) / 16];
