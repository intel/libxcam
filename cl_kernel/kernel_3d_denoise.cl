...
inline void weighted_average_subblock (__read_only image2d_t input, __write_only image2d_t output, float* sum_weight,
...
#pragma unroll
        for (int j = 0; j < 3; j++) {
            ...
            gradient = (float4)(ref_block[4 * i + 1][j].s2, ref_block[4 * i + 1][j].s2, ref_block[4 * i + 1][j].s2, ref_block[4 * i + 1][j].s2);
            gradient = fabs(gradient - ref_block[4 * i][j]) + fabs(gradient - ref_block[4 * i + 1][j]) + fabs(gradient - ref_block[4 * i + 2][j]) + fabs(gradient - ref_block[4 * i + 3][j]);
...
__kernel void kernel_3d_denoise ( float gain, float threshold,
                                  ...
                                  __read_only image2d_t input, __read_only image2d_t inputPrev1, __read_only image2d_t inputPrev2)
{
    ...
    weighted_average (input, output, sum_weight, true, observed_block, restored_block, gain, threshold, row_batch, col_batch);
#endif

#if REFERENCE_FRAME_COUNT == 2
    //printf("reference frame count 2! \n");
    weighted_average (input, output, sum_weight, true, observed_block, restored_block, gain, threshold, row_batch, col_batch);
    weighted_average (inputPrev1, output, sum_weight, false, observed_block, restored_block, gain, threshold, row_batch, col_batch);
#endif
    ...
    weighted_average (input, output, sum_weight, true, observed_block, restored_block, gain, threshold, row_batch, col_batch);
    weighted_average (inputPrev1, output, sum_weight, false, observed_block, restored_block, gain, threshold, row_batch, col_batch);
    weighted_average (inputPrev2, output, sum_weight, false, observed_block, restored_block, gain, threshold, row_batch, col_batch);
#endif
